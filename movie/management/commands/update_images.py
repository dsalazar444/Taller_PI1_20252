...
class Command(BaseCommand):
    ...
    def generate_and_download_image(self, client, movie_title, save_folder):
        ...
        response = client.images.generate(
         ...
         #Obtenemos la ruta de la imagen generada por la api, porque este nos
         #retorna varias imagenes en la lista response, entonces obtenemos la primera
        image_url = response.data[0].url

        # ✅ Prepare the filename and full save path
        image_filename = f"m_{movie_title}.png"
        #Creamos la ruta completa, que tiene la carpeta de destino/nombre_imagen
